...
def change_report(url_rep_1, url_rep_2):
    r1=(open_clean_var(url_rep_1).
        ...
        [['CODDRE', 'DRE', 'codooii','cod_unico','DNIDIRECTIVO','phone_number', 'statusdes','sec_gen', 'sec_met','envio']].
        copy()
       )
    r2=(open_clean_var(url_rep_2).
        query('cod_unico!="000"', engine='python')
        [['CODDRE', 'DRE', 'codooii','cod_unico','DNIDIRECTIVO','phone_number', 'statusdes','sec_gen', 'sec_met']].
        copy()
       )
    r2.columns=['CODDRE_2', 'DRE_2', 'codooii_2','cod_unico_2','DNIDIRECTIVO_2', 'phone_number_2', 'statusdes_2','sec_gen_2', 'sec_met_2']
    ...
    r3=r3[['CODDRE_2', 'DRE_2', 'codooii_2','DNIDIRECTIVO_2', 'phone_number_2', 'statusdes_2','sec_gen_2', 'sec_met_2','envio']].reset_index()
    r3.columns=['cod_unico','CODDRE', 'DRE', 'codooii','DNIDIRECTIVO','phone_number', 'statusdes','sec_gen', 'sec_met','envio']
...
def open_clean_var(url):
    ...
    df=df.rename({'TELEFONOPERSONAL':'phone_number','CODUGEL':'codooii'},axis=1)
    
    return df

def clean_numbers(df_o):
    df=df_o.copy()
    df["phone_l"]=df.phone_number.str.strip().str.len()
    df=df.query('phone_l==9 & phone_number!="999999999" & phone_number!="123456789"', engine='python').copy()
    df['phone_number']=df['phone_number'].astype(int)
    del df["phone_l"]
    
    return df

def cleaning_report_status_df(url, estado, tipologia=None,var_start=None, var_end=None, test=False, first_rep=False):
    if test==True:
        df_3=url.copy()
    else:
        if first_rep==True:
            df_3=open_clean_var(url)[['CODDRE', 'DRE', 'codooii','cod_unico','DNIDIRECTIVO','phone_number', 'statusdes','sec_gen', 'sec_met']].copy()
            df_3["envio"]=0
        else:
